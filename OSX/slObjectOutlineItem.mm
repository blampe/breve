...
- (void)getEval {
  ...
  if(mEval->type() == AT_INSTANCE) {
    ...
    for(n=0;n<childCount;n++) {
      if(childObjects[n]) {
        [childObjects[n] setInstance: (stInstance*)BRINSTANCE( mEval )->userData];
      }
    }
  }
}
...
- (NSString*)getValue {
  ...
  result = [NSString stringWithCString: cstr encoding:NSUTF8StringEncoding];
...
- (id)childAtIndex:(int)index {
  ...
  slObjectOutlineItem *childObject = childObjects[index];
  if( childObject ) {
    if( mEval->type() == AT_INSTANCE ) {
      stInstance *evalInstance = (stInstance*)BRINSTANCE( mEval )->userData;
      [childObject setInstance: evalInstance];
    }
  ...
  if( mEval->type() == AT_INSTANCE ) {
      ...
      childObject = [[slObjectOutlineItem alloc] initWithEval: &newEval name: @"super" withVar: NULL withOffset: -1 instance: instance];
      [childObject setEvalObject: object->super];
		}
    else {
      ...
      newTitle = [ [ NSString stringWithCString: var->name.c_str() encoding:NSUTF8StringEncoding] retain ];
      ...
      childObject = [[slObjectOutlineItem alloc] initWithEval: &newEval name: newTitle withVar: var withOffset: var->offset instance: evalInstance];
    }
  } 
    ...
    childObject = [[slObjectOutlineItem alloc] initWithEval: &newEval name: newTitle withVar: NULL withOffset: -1 instance: instance];
    [childObject setList: BRLIST( mEval ) index: index];
  } 
    ...
    childObject = [[slObjectOutlineItem alloc] initWithEval: &newEval name: newTitle withVar: NULL withOffset: off instance: instance];
  }
  
  if (childObjects[index] != nil) {
    [childObjects[index] release];
    childObjects[index] = nil;
  }
  childObjects[index] = childObject;
  return childObject;
}
